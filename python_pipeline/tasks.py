...
def run_spider():
    ...
    try:
        subprocess.run(['scrapy', 'crawl', 'event_spider', '-o', output_file], cwd='python_pipeline/event_scraper', check=True)
    ...
    try:
        ...
        for event in events:
            cursor.execute(
                ...
                """,
                (
                    event['title'], event['date'], event['location'], event['description'], event['link'], event.get('organizer', ''),
                    event['title'], event['date'], event['location'], event['description'], event.get('organizer', '')
                )
            )
            inserted_count += 1
